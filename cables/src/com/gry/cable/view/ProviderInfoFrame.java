...
public class ProviderInfoFrame extends JInternalFrame {
	...
	public ProviderInfoFrame(){
		super("供应商信息管理", false, true, false, true);
		Dimension screenSize = Toolkit.getDefaultToolkit().getScreenSize();
		this.setBounds(screenSize.width / 12, screenSize.height / 20,
				screenSize.width / 3*2, screenSize.height / 3*2);
		getContentPane().setLayout(new FlowLayout());
		JLabel jLabel = new JLabel("供应商信息管理");
		jLabel.setLocation(screenSize.width / 3-100, 0);
		jLabel.setSize(200, 30);
		jLabel.setFont(new Font("宋体", Font.PLAIN, 23));
		header = new String[]{"供应商单位","联系人","联系电话","地址","仓库地址","备注","id"};
		...
		panel.add(jLabel);
		JButton addCableButton = new JButton("添加供应商信息");	
		addCableButton.setLocation(screenSize.width / 3*2 -170,10);
		addCableButton.setSize(130, 20);
		addCableButton.addActionListener(new ActionListener() {
			
			@Override
			public void actionPerformed(ActionEvent e) {
				ProviderInfoAdd providerInfoAdd = new ProviderInfoAdd();
				MainFrame.getMainFrame().getContentPane().add(providerInfoAdd);
				providerInfoAdd.setVisible(true);
			}
		});
		...
		JLabel searchName1 = new JLabel("供应商单位：");
		...
		searchButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				//String[][] datas = null;
				data = providerInfoService.getProviderInfo(searchName.getText(),searchContact.getText());
		...
		delButton.addActionListener(new ActionListener(){//添加事件
            public void actionPerformed(ActionEvent e){
                ...
                {
                	rs = providerInfoService.deleteProviderInfo(Integer.valueOf((String) jTable.getValueAt(selectedRow, 6)));
                	if(rs == -1){
